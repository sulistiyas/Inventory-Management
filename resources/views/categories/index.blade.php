...
@section('content')
    ...
    @edit-category.window="handleEdit($event.detail)"
    @delete-category.window="handleDelete($event.detail)">
        <div class="page-header">
            ...
            <div class="page-header-actions">
                ...
                <button @click="openCreateCategory()" class="btn btn-primary btn-sm">
        ...
        <div
                                ...
                                <tr class="datatable-row">
                                    ...
                                    <td class="datatable-td datatable-td-actions">
                                        <div class="datatable-actions">
                                            <button @click="$dispatch('edit-category', item.id)" class="datatable-action-btn datatable-action-edit" title="Edit">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.75" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Z"/>
                                                </svg>
                                            </button>
                                            <button @click="$dispatch('delete-category', item.id)" class="datatable-action-btn datatable-action-delete" title="Delete">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.75" stroke="currentColor">
            ...
            {{-- ================================================================
            THE MODAL — uses <x-modal> component.
            The <form> wraps panel so submit works naturally.
            ================================================================ --}}
                        ...
                        <div class="modal-form">
                            ...
                            <template x-if="isEdit">
                                <div class="form-group">
                                    ...
                                    />
                                    <span class="form-hint">Kosongkan untuk generate otomatis dari nama.</span>
                                </div>
                            </template>
        
                            {{-- Description --}}
                            <div class="form-group">
                                <label for="cat-desc" class="form-label">Deskripsi</label>
                                <textarea
                                    id="cat-desc"
                                    class="form-textarea"
                                ...
                                <template x-if="hasError('description')">
                                    <div class="form-error">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><circle cx="12" cy="12" r="10"/><path stroke-linecap="round" stroke-linejoin="round" d="M12 8v4m0 4h.01"/></svg>
                                        <span x-text="getError('description')"></span>
                                    </div>
                                </template>
                            </div>
    ...
    <script>
    /**
     * Category page Alpine component.
     * Spreads the reusable modal() mixin in, then adds page-specific logic.
     */
    function categoryManager() {
        return {
            // ── Spread the reusable modal mixin ──────────────────────
            ...warehouseModal(),
            ...
            /** Fetch category data then open edit modal */
            async handleEdit(id) {
                try {
            ...
            handleDelete(category) {
                this.openDelete(category, {
                    title: 'Hapus Kategori',
                });
            },
    
            /** Form submit — routed to create or update */
            async handleSubmit() {
                if (this.isCreate) {
                    await this.submit(
                        () => this.postTo('/api/categories', this.form),
                        () => this._reload(),
                    );
                } else if (this.isEdit) {
                    await this.submit(
                        () => this.putTo(`/api/categories/${this.record.id}`, this.form),
                        () => this._reload(),
                    );
                } else if (this.isDelete) {
                    await this.submit(
                        () => this.deleteTo(`/api/categories/${this.record.id}`),
                        () => this._reload(),
                    );
                }
            },
    
            /** Refresh page data after CUD operations */
            _reload() {
